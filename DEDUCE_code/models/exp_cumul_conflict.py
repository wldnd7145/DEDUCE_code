# Exp: Cumulative Conflict Score
# Replace 1/(1+0.001*F) in LUM with accumulated per-parameter conflict score
# score_i += 1 when g_old_i * g_new_i < 0, normalized to [0,1], persists across tasks
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp: Cumulative Conflict Score')
...
class ExpCumulConflict(ContinualModel):
    NAME = 'exp_cumul_conflict'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ExpCumulConflict, self).__init__(backbone, loss, args, transform)
        ...
        self.checkpoint = {}
        self.fish = {}
        # Cumulative conflict score per parameter (persists across tasks)
        self.conflict_score = {}
        for name, param in self.net.named_parameters():
            self.fish[name] = torch.zeros_like(param).to(self.device)
            self.conflict_score[name] = torch.zeros_like(param).to(self.device)
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        if self.buffer_grad is not None:
            # Build per-param new gradient dict and update conflict scores
            new_grad_dict = {}
            for (name, _), g in zip(self.net.named_parameters(), grads):
                if g is not None:
                    new_grad_dict[name] = g.clone()
                    # Accumulate conflict score
                    if self.buffer_grad_dict is not None and name in self.buffer_grad_dict:
                        conflict = (self.buffer_grad_dict[name] * g < 0).float()
                        self.conflict_score[name] += conflict
            ...
            if left <= right:
                self.lum_cumul_conflict(inputs=inputs, labels=labels)
                self.times += 1
    ...
    def lum_cumul_conflict(self, inputs, labels):
        """LUM with cumulative conflict score as weighting"""
        ...
        for (mn, mp), (tn, tp) in zip(self.net.named_parameters(), self.temp.named_parameters()):
            ...
            score = self.conflict_score[mn]
            s_max = score.max()
            if s_max > 0:
                w = score / s_max
            else:
                w = torch.ones_like(mp)

            with torch.no_grad():
                mp.add_(self.delta * w * (norm_update + 0.001 * torch.randn_like(norm_update)))
