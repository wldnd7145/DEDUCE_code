# Exp 3: Moving average cosine similarity for detection
# Instead of single-batch dot product, use running average of cosine similarity over last 10 batches
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp3: Moving Average Detection')
...
class Exp3MovingAvg(ContinualModel):
    NAME = 'exp3_moving_avg'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(Exp3MovingAvg, self).__init__(backbone, loss, args, transform)
        ...
        self.times = 0
        # Moving average window of cosine similarities
        self.cos_sim_history = deque(maxlen=10)
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        if self.buffer_grad is not None:
            ...
            cos_sim = torch.dot(gradients, self.buffer_grad) / (
                torch.norm(gradients) * torch.norm(self.buffer_grad) + 1e-8)
            self.cos_sim_history.append(cos_sim.item())

            # Activate LUM if moving average of cosine similarity is negative
            if len(self.cos_sim_history) >= 1:
                avg_cos = sum(self.cos_sim_history) / len(self.cos_sim_history)
                if avg_cos < 0:
