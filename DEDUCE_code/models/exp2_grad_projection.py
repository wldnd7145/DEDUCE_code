# Exp 2: PCGrad-style gradient projection instead of LUM
# When gradient conflict detected, project current gradient to remove conflicting component
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp2: Gradient Projection')
...
class Exp2GradProjection(ContinualModel):
    NAME = 'exp2_grad_projection'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(Exp2GradProjection, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        if self.buffer_grad is not None:
            ...
            dot = torch.dot(gradients, self.buffer_grad)
            if dot < 0:
                # PCGrad: project out conflicting component
                proj = dot / (torch.dot(self.buffer_grad, self.buffer_grad) + 1e-8)
                gradients = gradients - proj * self.buffer_grad
                # Apply projected gradient back to parameters
                offset = 0
                for param in self.net.parameters():
                    numel = param.numel()
                    param.grad = gradients[offset:offset+numel].view(param.shape).clone()
                    offset += numel
                projected = True
                self.times += 1

        # Re-forward for loss computation (with EWC)
        if not projected:
            self.opt.zero_grad()
        outputs, features = self.net(inputs, feature_list=current_features)
        loss = self.loss(outputs, labels)
    ...
    def cal_buffer(self, task_number):
        ...
        for i in unique_tasks:
            ...
            for batch_idx in range(num_batches):
                ...
                self.opt.zero_grad()
                buf_outputs, feas = self.net(task_inputs[start_idx:end_idx])
                buffer_loss = self.loss(buf_outputs, task_labels[start_idx:end_idx])
