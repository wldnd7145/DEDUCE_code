# Exp A + No EWC: LUM + Gradient Projection at learning step, but without EWC penalty
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp A + No EWC')
...
class ExpANoEwc(ContinualModel):
    NAME = 'exp_a_no_ewc'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ExpANoEwc, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        # Gradient projection (same as Exp A)
        if self.buffer_grad is not None:
