# Exp: Soft Conflict Weight
# Replace 1/(1+0.001*F) in LUM with soft conflict weight w_i = max(0, -g_old_i * g_new_i), normalized to [0,1]
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp: Soft Conflict Weight')
...
class ExpSoftConflict(ContinualModel):
    NAME = 'exp_soft_conflict'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ExpSoftConflict, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        if self.buffer_grad is not None:
            ...
            if left <= right:
                ...
                self.lum_soft_conflict(inputs=inputs, labels=labels, new_grad_dict=new_grad_dict)
                self.times += 1
    ...
    def lum_soft_conflict(self, inputs, labels, new_grad_dict):
        """LUM with soft conflict weight instead of Fisher scaling"""
        ...
        # Step 2: apply update with soft conflict weight
        for (mn, mp), (tn, tp) in zip(self.net.named_parameters(), self.temp.named_parameters()):
            ...
            # Soft conflict weight: w_i = max(0, -g_old_i * g_new_i), normalized
            if self.buffer_grad_dict is not None and mn in self.buffer_grad_dict and mn in new_grad_dict:
                conflict = -self.buffer_grad_dict[mn] * new_grad_dict[mn]
                w = torch.clamp(conflict, min=0)
                w_max = w.max()
                if w_max > 0:
                    w = w / w_max
                else:
