# Exp: g_old direction unlearning
# Instead of gradient ascent (-grad_ce), unlearn in the direction of buffer gradient g_old
# theta' = theta - delta * 1/(1+0.001*F) * g_old
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp: g_old direction unlearning')
...
class ExpGoldDirection(ContinualModel):
    NAME = 'exp_gold_direction'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ExpGoldDirection, self).__init__(backbone, loss, args, transform)
    ...
    def lum_gold_direction(self):
        """Unlearn in the direction of buffer gradient g_old.
        theta' = theta - delta * 1/(1+0.001*F) * g_old"""
        if self.buffer_grad_dict is None:
            return
        with torch.no_grad():
            for name, param in self.net.named_parameters():
                if name in self.buffer_grad_dict:
                    identity = torch.ones_like(self.fish[name])
                    fisher_scale = 1.0 / (identity + 0.001 * self.fish[name])
                    param.add_(-self.delta * fisher_scale * self.buffer_grad_dict[name])
