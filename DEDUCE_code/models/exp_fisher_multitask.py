# Exp: Fisher Multi-task (weak selectivity, c=0.001)
# Replace LUM gradient ascent with Fisher-weighted role split:
# Important params (high Fisher) -> follow buffer gradient
# Unimportant params (low Fisher) -> follow current batch gradient
# w_F = 0.001*F / (1 + 0.001*F)
# theta' = theta - delta * (w_F * grad_CE_buffer + (1-w_F) * grad_CE_current)
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp: Fisher Multi-task (weak)')
...
class ExpFisherMultitask(ContinualModel):
    NAME = 'exp_fisher_multitask'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    FISHER_C = 0.001  # weak selectivity

    def __init__(self, backbone, loss, args, transform):
        super(ExpFisherMultitask, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        if self.buffer_grad is not None:
            ...
            if left <= right:
                # Fisher-weighted multi-task update (replace LUM)
                self.fisher_multitask_update(inputs=inputs, labels=labels)
                self.times += 1

        # === LEARN (DER++ + EWC, same as official) ===
        self.opt.zero_grad()
        outputs, features = self.net(inputs, feature_list=current_features)
        loss = self.loss(outputs, labels)
    ...
    def fisher_multitask_update(self, inputs, labels):
        """Fisher-weighted role split update.
        Important params follow buffer CE gradient, unimportant follow current CE gradient.
        theta' = theta - delta * (w_F * grad_buf + (1-w_F) * grad_cur)
        """
        if self.buffer.is_empty():
            return

        # Compute current batch CE gradient
        self.opt.zero_grad()
        outputs, _ = self.net(inputs)
        loss_cur = self.loss(outputs, labels)
        loss_cur.backward()
        grad_cur_dict = {}
        for name, param in self.net.named_parameters():
            if param.grad is not None:
                grad_cur_dict[name] = param.grad.clone()
        self.opt.zero_grad()

        # Compute buffer CE gradient
        ...
        for name, param in self.net.named_parameters():
            if param.grad is not None:
                grad_buf_dict[name] = param.grad.clone()
        self.opt.zero_grad()

        # Apply Fisher-weighted update
        with torch.no_grad():
            for name, param in self.net.named_parameters():
                if name not in grad_cur_dict or name not in grad_buf_dict:
                    continue
                F_p = self.fish[name]
                w_F = (self.FISHER_C * F_p) / (1.0 + self.FISHER_C * F_p)
                update = w_F * grad_buf_dict[name] + (1.0 - w_F) * grad_cur_dict[name]
                param.add_(-self.delta * update)
