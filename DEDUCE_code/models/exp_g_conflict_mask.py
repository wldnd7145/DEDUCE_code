# Exp G: Conflict Mask - replace Fisher scaling in LUM with per-parameter conflict mask
# mask=1 where g_old_i * g_new_i < 0, mask=0 otherwise
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp G: Conflict Mask')
...
class ExpGConflictMask(ContinualModel):
    NAME = 'exp_g_conflict_mask'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ExpGConflictMask, self).__init__(backbone, loss, args, transform)
        ...
        self.buffer_grad_dict = None  # per-parameter buffer gradients for conflict mask
        self.times = 0
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        if self.buffer_grad is not None:
            ...
            if left <= right:
                # Compute per-parameter new gradient for conflict mask
                new_grad_dict = {}
    ...
    def lum_conflict_mask(self, inputs, labels, new_grad_dict):
        ...
        Only unlearn parameters where buffer gradient and new gradient conflict."""
        ...
        # Step 2: apply update with conflict mask instead of 1/(1+0.001*F)
        for (mn, mp), (tn, tp) in zip(self.net.named_parameters(), self.temp.named_parameters()):
            ...
            # Conflict mask: mask=1 where g_old_i * g_new_i < 0
    ...
    def cal_buffer(self, task_number):
        """Returns (flat_gradient, per_param_gradient_dict)"""
        classes_per_task = 10
