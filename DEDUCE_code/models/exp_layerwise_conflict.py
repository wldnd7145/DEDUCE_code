# Exp: Layer-wise Conflict
# Adjust delta per layer based on cosine similarity between g_old and g_new per layer
# conflict_intensity_l = max(0, -cos(g_old_l, g_new_l))
# theta'_l = theta_l + delta * conflict_intensity_l * 1/(1+0.001*F_l) * norm_update_l
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description='Exp: Layer-wise Conflict')
...
class ExpLayerwiseConflict(ContinualModel):
    NAME = 'exp_layerwise_conflict'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']

    def __init__(self, backbone, loss, args, transform):
        super(ExpLayerwiseConflict, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs, task_id=None, gradients=None, unlearn_flag=None):
        ...
        if self.buffer_grad is not None:
            ...
            if left <= right:
                self.lum_layerwise(inputs=inputs, labels=labels, new_grad_dict=new_grad_dict)
                self.times += 1
    ...
    def lum_layerwise(self, inputs, labels, new_grad_dict):
        """LUM with per-layer conflict intensity scaling delta"""
        # Step 1: gradient ascent on temp model (same as original)
        ...
        # Step 2: apply update with layer-wise conflict intensity
        for (mn, mp), (tn, tp) in zip(self.net.named_parameters(), self.temp.named_parameters()):
            ...
            # Compute per-layer conflict intensity
            conflict_intensity = 1.0  # default
            if self.buffer_grad_dict is not None and mn in self.buffer_grad_dict and mn in new_grad_dict:
                g_old_flat = self.buffer_grad_dict[mn].view(-1)
                g_new_flat = new_grad_dict[mn].view(-1)
                g_old_norm = g_old_flat.norm()
                g_new_norm = g_new_flat.norm()
                if g_old_norm > 1e-8 and g_new_norm > 1e-8:
                    cos_sim = torch.dot(g_old_flat, g_new_flat) / (g_old_norm * g_new_norm)
                    conflict_intensity = max(0.0, -cos_sim.item())

            # Original Fisher scaling
            identity = torch.ones_like(self.fish[mn])
            fisher_scale = 1.0 / (identity + 0.001 * self.fish[mn])

            with torch.no_grad():
                mp.add_(self.delta * conflict_intensity * torch.mul(fisher_scale, norm_update + 0.001 * torch.randn_like(norm_update)))
